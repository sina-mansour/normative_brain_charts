# Mathematics of kernel normative modelling

In this section, we provide a brief overview of methods from Graph Signal Processing that are used to defined the kernel normative model.

---

## GSP paradigm:

The GSP approach is a method that can convert an arbitrary *signal* defined over a network structure (refered to as the **graph signal**) from it's original domain to a graph frequency domain.
...
Here, we'll take a short detour to describe how GFT can be used to transform signal from it's original domain to the graph frequency domain.

Essentially, we build an orthonormal basis (a set of eigenmodes that are orthogonal and have a unit norm) for a $N_v$ dimensional space of all possible graph signals. Hence, theoritacilly, this basis can explain any arbitrary graph signal. In other words, with the complete basis (of $N_v$ eigenmodes), any graph signal can be described as a linear combination of the eigenmodes.

This basis is generated from the network's adjaceny matrix $A \in R^{N_v} \times R^{N_v}$ and eigenmodes are ordered based on the graph frequecy.
...
*Footnote*: In GSP literature, this matrix (and other similar matrices) is refered to as the Graph Shift Operator. As it basically can be viewed as a rule to shift information on the graph. Basically, $L^{sim}$ describes a shift operation in which information can be sourced from any vertex, and is distributed according to the connectivity weights.

Now, this laplacian matrix is decomposed using Singular Value Decomposition (SVD) to result the eigenmodes (also refered to as eigendecompositions):

$$L = V \Lambda V^{-1}$$

In this decomposition $\Lambda \in R^{N_v} \times R^{N_v}$ is a diagonal matrix with the diagonal values being the eigenvalues, and $V  \in R^{N_v} \times R^{N_v}$ is a list of $N_v$ eigenvectors, each associated with an eigenvalue. As the eigenvectors are ordered by their corresponding eigenvalue, starting eigenmodes are topologically smooth (vertices that are connected are more likely to have similar values) and are hence referred to as lower graph frequencies. Similarly the last eigenmodes are topologically chaotic/varying and are considered to explain higher graph frequencies. The kernel visualizations of our brain eigenmodes depicted this.
...
So $\overrightarrow{\tilde{X}}$ is called the fourier transformed signal in frequency domain. This transformed signal contains all the information about the original signal and can in fact be used to reconstruct the original signal:
...
Very similar to the idea of filtering discrete time series using a fourier transform, graph signals can be filtered by a graph fourier transform (in fact the former is a special case of the later, see [this](https://ieeexplore.ieee.org/document/8347162/)).

To explain succinctly, a graph signal $\overrightarrow{X}$ can be transormed to the graph frequency domain as described above, $V^T \overrightarrow{X}$. Next a frequency filter, in the form of a diagonal matrix $F \in R^{N_v} \times R^{N_v}$ can be used to filter the signal in frequency domain, $F V^T \overrightarrow{X}$. To be exact, $F$ is diagonal matrix for which the diagonal elements are all in the range of $[0,1]$. For instance, $F$ can be a diagonal matrix with first 10 diagonal elements equal to one, and the rest equal to zero. This would be an example of a low-pass graph filter because only the lower frequencies survive filtering.
...
- The subsequent normative modelling step will be less computational as instead of requiring $N_v$ separate normative fits, we only need $N_{LP}$ separate fits.

---

This not very short introduction on GSP brings us to the main topic of discussion, which is how we can use the GSP paradigm to perform smooth high-resolution normative modelling of the developmental trajectories of the brain.

In the ensuing section we'll describe:

- The adjacency matrix used for GSP on the brain
- How normative modelling can be fitted over brain eigenmodes
...
### The adjacency structure on brain
...
Next, we compute SVD of the laplacian matrix. To reduce computations, instead of all eigenvectors, we only derive the first $N_{LP} = 2000$ eigenmodes. Hence, we can use this parial decomposition $V_{LP}  \in R^{N_v} \times R^{N_{LP}}$ to approximate a full signal reconstruction. This was any brain signal $\overrightarrow{X} \in R^{N_v}$ can be encoded into a dimensionality reduced spectral domain encoding $\overrightarrow{\tilde{X}}_{LP} \in R^{N_{LP}}$:
...
To put simply, for every eigenmode, the mean and standard deviation across subjects is modelled as a function of the covariates (age, sex, and site). Hence, for normative kernel modelling, we essentially fit $N_{LP}$ separate normative models to assess variations of different spectral frequencies of thickness.
...
The reason that this signal explains the mean thickness operation is that for every participant $i$ with high-resolution thichkess information of $\overrightarrow{T_{(p=i)}}$ ($i$th column of $T$) the mean thickness can be computed by the dot product of $\overrightarrow{X}_{MT} . \overrightarrow{T_{(p=i)}}$.

So to generalize, for any brain signal query $\overrightarrow{X_Q}$ that describes a linear operation based on high-resolution values of thickness (e.g. mean thickness, thickness of a region, a thickness contrast or comparison), that linear operation can be computed for all individuals in the group with a matrix multiplication with the signal vector:

$$\overrightarrow{T_{X_Q}} = \overrightarrow{X_Q} T$$

In the case of mean thickness, $\overrightarrow{T_{X_{MT}}}$ would be a vector of length $N_p$ that stores mean thickness of all individuals.
...
The normative mean of low-pass filtered query signal operated on a hypothetical high-resolution thickness vector $\overrightarrow{T}$ can thus be written as:
...
Again, we know that the low-pass filtered approximation of the query signal (as a random variable) can be written as a linear combination of the eigennorms which are assumed to be normally distributed random variables. As such, we can use statistical rules to compute variance of a posterior normal distribution from the linear combination of variances and covariances of the priors:
...
Now this makes it a bit complicated to accurate compute the variance of the query signal from the eigennorms. The main issue is that for the eigennorms, we have modelled the standard deviation ($\sigma$) of each eigenmode as a function of the covariates (age, sex, site). Variance can be easilly computed from the standard deviation. However, the covariances were not modelled as a function the covariates. One solution would be to construct a more detailed model that also fits covariances of the eignenorms (potentially a future to do!), but for now we could try to come up with an approximation of the covariance structure.
...
Interestinglym $G V_{LP}$ is equivalent to applying spatial smoothing on every eigenmode separately.
